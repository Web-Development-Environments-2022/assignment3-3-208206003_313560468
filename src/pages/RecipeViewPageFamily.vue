<template>
  <div>
    <RecipeView :recipe="recipe" :family_recipe="true"></RecipeView>
  </div>
</template> 
...
export default {
  ...
  async created() {
    ...
    if (localStorage.getItem("familyRecipes") != undefined) {
      let familyRecipes = JSON.parse(localStorage.getItem("familyRecipes"));

      for (let i = 0; i < familyRecipes.length; i++) {
        if (familyRecipes[i].id === this.$route.params.recipeId) {
          let {
            instructions,
            ingredients,
            readyInMinutes,
